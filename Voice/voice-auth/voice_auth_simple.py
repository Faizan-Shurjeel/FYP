...
class VoiceAuthSystem:
    ...
    def register_user(self, username, num_samples=3):
        ...
        if username in self.users:
            overwrite = input(
                f"{Fore.YELLOW}User '{username}' already exists. Overwrite? (y/n): "
            )
        ...
        print(
            f"{Fore.YELLOW}📝 We'll record {num_samples} voice samples for better accuracy.\n"
        )
        ...
        for i in range(num_samples):
            ...
            self.record_audio(duration=5, filename=str(audio_file))
    ...
    def verify_user(self, username, test_audio=None):
        ...
        if test_audio is None:
            ...
            test_audio = self.record_audio(duration=5, filename="temp/verify_temp.wav")
        ...
        similarities = [
            np.dot(test_embedding, stored_emb) for stored_emb in stored_embeddings
        ]
    ...
    def list_users(self):
        ...
        for i, (username, info) in enumerate(self.users.items(), 1):
            reg_date = datetime.fromisoformat(info["registered_date"]).strftime(
                "%Y-%m-%d %H:%M"
            )
...
def main_menu():
    ...
    while True:
        ...
        if choice == "1":
            ...
            system.register_user(username, num_samples=5)
